...
python $TAGGERPATH/tagger/bin/trainer.py \
  ...
  --parameters="save_summary=false,"`
               ...
               `"learning_rate_values=[1.0,0.2,0.04],learning_rate_boundaries=[300000,400000],"`
               `"save_checkpoint_steps=2000,early_stopping=10,plot_step=1000,"`
               `"device_list=$DEVICE,clip_grad_norm=1.0"
#,embedding=$EMBPATH/embedding.txt,script=validation.sh
